...
;; length: 128, 192, 256
(defmethod make-algo-params :aes-key-gen
...
(defmethod make-algo-params :ecdsa [type {:keys [hash] :or {hash "SHA-256"}}]
...
(defmethod make-algo-params :ec-key-gen
...
(defmethod make-algo-params :rsa-pss [type {:keys [salt-length] :or {salt-length 32}}]
...
;; name: RSASSA-PKCS1-v1_5, RSA-PSS, RSA-OAEP
;; hash: SHA-256, SHA-384, SHA-512
(defmethod make-algo-params :rsa-hashed-import
...
(defmethod make-algo-params :hmac-key-gen
...
;; salt: bytes
;; info: bytes
(defmethod make-algo-params :hkdf
...
;; salt: 16 bytes
;; iterations: int
(defmethod make-algo-params :pbkdf2
...
(comment
  (do
    (def res (atom nil))
    (-> (promesa.core/let [key (create-key (algo-params :aes-gcm-key-gen) :encrypt :extractable true)]
          (reset! res key))
        (promesa.core/catch #(reset! res %))))
  (do
    (-> (promesa.core/let [ex (export-key @res :format :jwk)]
          (reset! res ex))
        (promesa.core/catch #(reset! res %))))
  (do
    (-> (promesa.core/let [key (create-key (algo-params :aes-gcm-key-gen) :encrypt :from [:import :jwk @res])]
          (reset! res key))
        (promesa.core/catch #(reset! res %)))))
